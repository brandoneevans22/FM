...
function respond() {
  ...
  if(request.text && botRegex.test(request.text)) {
    ...
    this.res.writeHead(200);
    postMessage("http://www.youtube.com/"+request.text.substring(6,request.text.length));
    this.res.end();
  } 
  else if(request.text && botRegexFYt.test(request.text)) {
    this.res.writeHead(200);
    postMessage("https://www.youtube.com/results?search_query="+request.text.substring(4,request.text.length)+"+"request.text.substring(13,request.text.length));
    this.res.end();
  } 
